  ...
  */
  show: function(roster_item,nick) {
    ...
    if (Modernizr.canvas)
    {
      var img_src = null;
      var img = false;
      if (roster_item.vcard && $(roster_item.vcard).children().length > 0)
      {
      ...
      if (img)
      {
        li_clone.find('.useronline')
          .prepend('<canvas></canvas>');
        var li_canvas = li_clone.find('canvas');
        this.drawAvatar(li_canvas, {src: img_src, data: img, width: 30, height: 30})
      }
      else
      {
        this.drawAvatar(online_avatar, {src: img_src, data: false, width: 30, height: 30})
      }
    }
    else
    {
  ...
  // avatar is an image object with src, height, width, and optional
  // data and sha1 attrs.  The data attr will exist if it's HTML5 and
  // the sha1 will exist if it's less than HTML5
  drawAvatar: function(img_elem, avatar) {
    if (avatar.data)
    {
      var image = new Image();
      image.onload = function () {
        var ctx = img_elem.get(0)
          .getContext('2d');
        ctx.drawImage(avatar.data,0,0);
        }
      image.src = avatar.src;
    }
    else
    {
      img_elem.attr("src", avatar.src.join(""));
      ...
      if (Speeqe.IE6)
      {
        ...
        var filter = ["progid:DXImageTransform.Microsoft.AlphaImageLoader(src='",this.service,"'?sha1=",avatar.sha1,"',sizingMethod='scale')"];
